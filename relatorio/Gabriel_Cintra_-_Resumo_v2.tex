...
Esta versão reúne a matriz P, a comparação entre emissões e VAB e duas seleções de ligações: a árvore máxima bilateral e a arborescência máxima dirigida, sem agregação de atividades. Os resultados são preliminares e exploratórios; as definições e hipóteses metodológicas estão no README do repositório.
...
\begin{figure}[p]
\centering
\includegraphics[width=\textwidth,height=0.33\textheight,keepaspectratio]{imagens/03_arvore_maxima_v2.png}
\caption{Árvore máxima bilateral: maximiza a soma de $P_{ij}+P_{ji}$ sem ciclos. Mantém 67 atividades (uma isolada) e 65 ligações, preservando 30,58\% do peso intersetorial. Área: emissões próprias; espessura: peso bilateral; cor: diversidade de destinos na rede original, do roxo ao amarelo.}
\par\medskip
\includegraphics[width=\textwidth,height=0.33\textheight,keepaspectratio]{imagens/04_arborescencia_maxima_v2.png}
\caption{Arborescência máxima dirigida: maximiza a soma dos pesos $P_{ij}$, com 65 setas e 19,96\% do peso intersetorial. As setas ligam a emissora ao destino da demanda final; cada nó conectado recebe uma entrada, exceto a raiz, escolhida pela otimização. Mantêm-se as escalas da figura anterior. Layouts Kamada--Kawai; posições e caminhos não representam distâncias econômicas ou etapas produtivas.}
\end{figure}
